@testset verbose=false "StateHole" begin
    @testset "convert, isfilled and get_rule" begin
        root_stateless = UniformHole(BitVector((1, 1, 0, 0, 0)),
            [  # domain size 2
                UniformHole(BitVector((1, 0, 0, 0, 0)),
    ...
    @testset "is_filled (empty domain)" begin
        hole = HerbConstraints.StateHole(
            HerbConstraints.StateManager(), UniformHole(BitVector((0, 0, 0)), []))
