...
function HerbConstraints.grammar_to_ASP(grammar::AbstractGrammar)
    ...
    for (const_ind, constraint) in enumerate(grammar.constraints)
        output *= "% $(constraint)\n"
        output *= constraint_to_ASP(constraint, const_ind)
        output *= "\n"
    end
    return output
end
...
```
"""
function HerbConstraints.constraint_to_ASP(constraint::Forbidden, constraint_index::Int64)
    tree, _ = constraint_rulenode_to_ASP(constraint.tree, 1)
...
```
"""
function HerbConstraints.constraint_to_ASP(constraint::Contains, ::Int64)
...
```
"""
function HerbConstraints.constraint_to_ASP(constraint::Unique, ::Int64)
...
```
"""
function HerbConstraints.constraint_to_ASP(constraint::ContainsSubtree, constraint_index::Int64)
    tree, _ = constraint_rulenode_to_ASP(constraint.tree, 1)
...
```
"""
function HerbConstraints.constraint_to_ASP(constraint::Ordered, ::Int64)
