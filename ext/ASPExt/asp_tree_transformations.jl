...
"""
function HerbConstraints.rulenode_to_ASP(io::IO, rulenode::AbstractRuleNode, node_index::Int64; is_constraint=false)
    if is_constraint
        print(io, "node(X", node_index, ",")
        domain_to_asp(io, rulenode)
        print(io, ")")
        HerbConstraints.enforce_varnode_equality(io, rulenode, node_index)
    else
        print(io, "1 { node(", node_index, ",")
        domain_to_asp(io, rulenode)
        print(io, ") } 1.\n")
    end
    ...
    for (child_ind, child) in enumerate(get_children(rulenode))
        if is_constraint
            print(io, ",child(X$(parent_index),$(child_ind),X$(node_index)),")
        else
            print(io, "child($(parent_index),$(child_ind),$(node_index)).\n")
        end
        node_index = rulenode_to_ASP(io, child, node_index; is_constraint)
    end

    return node_index
end

function HerbConstraints.rulenode_to_ASP(rulenode::AbstractRuleNode, node_index::Int; is_constraint=false)
    io = IOBuffer()
    node_index = HerbConstraints.rulenode_to_ASP(io, rulenode, node_index; is_constraint)
    return String(take!(io)), node_index
end

function Base.show(io::IO, ::MIME"text/clingo", rn::AbstractRuleNode)
    node_index = get(io, :node_index, 1)
    is_constraint = get(io, :is_constraint, false)
    output, node_index = HerbConstraints.rulenode_to_ASP(rn, node_index; is_constraint)
    print(io, output)
    return nothing
end
...
```
"""
function domain_to_asp(io::IO, rulenode::RuleNode)
    print(io, get_rule(rulenode))
    return nothing
end

function domain_to_asp(io::IO, rulenode::Union{UniformHole,DomainRuleNode,StateHole})
    print(io, "(")
    join(io, sort(findall(rulenode.domain)), ";")
    print(io, ")")

    return nothing
end

function domain_to_asp(io::IO, ::VarNode)
    print(io, "_")
    return nothing
end
...
```
"""
function HerbConstraints.constraint_rulenode_to_ASP(io::IO, rulenode::AbstractRuleNode, node_index::Int)
    return rulenode_to_ASP(io::IO, rulenode, node_index; is_constraint=true)
end
function HerbConstraints.constraint_rulenode_to_ASP(rulenode::AbstractRuleNode, node_index::Int)
    io = IOBuffer()
    node_index = HerbConstraints.rulenode_to_ASP(io, rulenode, node_index; is_constraint=true)
    return String(take!(io)), node_index
end
...
"""
function HerbConstraints.enforce_varnode_equality(io::IO, rn::AbstractRuleNode, idx::Int)
